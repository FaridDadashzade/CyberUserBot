...
CmdHelp('dogbin').add_command(
    'paste', '<söz/yanıtlama>', 'Dogbin işlədərək yapışdırılmış və ya qısadılmış url yaratma (https://del.dog/)'
).add_command(
    'getpaste', None, 'Dogbin link məzmuna mətn axtarır (https://del.dog/)'
).add()
