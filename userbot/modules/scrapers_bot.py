...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./CyberSticker.png"):
                os.remove("./CyberSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("CyberSticker.png", "png")
            Resim = "CyberSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' CyberSticker.png")
            os.remove(sticker)
            Resim = "CyberSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} CyberThumb.jpg")
            os.remove(Resim)
            Resim = 'CyberThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./cybrrmeme.png"):
            os.remove("./cybermeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./cybermeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
...
CmdHelp('scrapers_bot').add_command(
    ...
    'meme', '<font> <üst;alt>', 'Fotoya yazı əlavə edin. İstəyirsinizsə font böyüklüyünüdə  yaza bilərsiz.', 'meme 30 cyber'
).add_command(
