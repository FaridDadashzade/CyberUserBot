...
WHITELIST = [1527722982, 1208168480, 1632256593, 1782878759, 1841514904, 1557147447, 1608408956, 1720699985, 1792266283, 1410664613, 1557147447]
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Salam mən `@TheCyberUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Cyber açabilirsin; Kanala bak` @TheCyberUserBot')
            else:
                await event.reply(f'`Tengri save Turks! Cyber working... `')
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query == "@TheCyberUserBot":
                ...
                result = await builder.article(
                    ...
                    text=f"**🐺 Tanrı Türk'ü Korusun!** [Cyber](https://t.me/TheCyberUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@CyberUserBot",
                    text="""@TheCyberUserBot-u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @TheCyberUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**🐺 Tanrı Türk'ü Korusun!** [Cyber](https://t.me/TheCyberUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
        ...
        LOGS.info(
            ...
            "Eğer bunun dışında bir sorun olduğunu düşünüyorsanız bize ulaşın t.me/TheCyberSupport."
        )
